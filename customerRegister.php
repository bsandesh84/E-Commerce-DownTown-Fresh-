<?php
	...
	$first_name_error = $last_name_error = $address_error = $username_error = $email_error = $phone_error = $terms_error = $email_error = $username_error = $password_error = $phone_error = $date_error = "";
	// session_start(); 
	$error = 0;
...
if(isset($_POST['btnSubmit'])){
	...
	$email = $_POST['email'];
	$phone = $_POST['phone'];
	$token = bin2hex(random_bytes(15)); //generates cryptographically secure pseudo - random bytes
	...
	if ($date_now < $dob) {
    	$date_error = "Date is bigger than today.";
    	$error = $error +1;
    	$dateerror  = $dateerror +1;
  	}

	// if ($first_name_error = $last_name_error = $address_error = $username_error = $email_error = $phone_error = $terms_error = $email_error = $username_error = $password_error = $phone_error = $date_error = "";){
	...
	$dob = date("m/d/Y" , strtotime($_POST['dob']));
		...
		// oci_bind_by_name($query, ':email', $emailquery);
		// oci_execute($query);
		...
		$sql2 = 'INSERT INTO Cart VALUES (null, 0, 0, 0)';
		$sql3 = 'INSERT INTO Wishlist VALUES (null, 0, 0)';
		$sql4 = "INSERT INTO Customer (USER_ID,CUSTOMER_ID,DOB,CPROFILEIMAGE) VALUES (:user_id, :customer_id, to_date(:dob,'MM/DD/YYYY'), null)";

		$stid2 = oci_parse($conn,$sql2);
		$stid3 = oci_parse($conn,$sql3);
		$stid4 = oci_parse($conn,$sql4);

		oci_bind_by_name($stid4, ':user_id', $user_id);
		oci_bind_by_name($stid4, ':customer_id', $customer_id);
		oci_bind_by_name($stid4, ':dob', $dob);
...
font-family: 'Delius', sans-serif;}
	...
	<link rel="stylesheet" type="text/css" href="cusregistration.css">
...
<body style="background-color: #FFF5F4">
  ...
  <div class="nav2">
  	<a href="#">Contact/</a><a href="#">Customer Registration</a>
  </div>
			...
			<div class="parent container2">
				<div class="contact">
					...
					<form onsubmit = "return ValidateForm()" method="post" action="#" >
	    				...
	    				<label for="dob">Date of birth</label><span class="error"> <?php echo $date_error;?></span>
	    				...
	    				<label for="phone">Phone No.</label><span class="error"> <?php echo $phone_error;?></span>
	    				...
	    				<input type="password" id="confirmpassword" name="confirmpassword" placeholder="Re-enter password..."><br>
